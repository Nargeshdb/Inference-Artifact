...
To run the artifact, execute the following command: (TODO(MS): doesn't this assume they have already run `docker load` or something like that to load the image?)
...
(TODO(MS): I think we should remove this sentence as it is now redundant) Third, there are slight differences in the numbers reported compared to the draft we originaly submitted, we have updated these numbers, and the artifact includes the revised version.

### Generate Numbers for Table 2

(TODO(MS): shouldn't this paragraph go in the C# implementation section?) Table 2 in our paper presents the number of warnings generated by RLC# in two scenarios: without annotations and with inferred annotations. These numbers are presented in the second and third columns of the table. The remaining columns in Table 2 involve manual analysis of the warnings and cannot be generated automatically. We offer this information separately in a Google Doc [Manual-Report](https://docs.google.com/spreadsheets/d/1qEQyj2kmLOnURmDFiHAPnu-vR4NbAUIwOliGqopLqnE/edit?usp=sharing).  (TODO(MS): do **not** leave any results in external Google Docs.  The reviewers cannot trust these documents as they might get modified after the deadline.  Everything should be included as a PDF or Excel file _within_ the artifact.)
...
TODO(MS): will it be completely obvious how the output of the script corresponds to the table?  If not, give some guidance. (NS) This part is clear I think! The output contains all the information mentioned in the paper.  (MS) Okay this TODO can be deleted then.
